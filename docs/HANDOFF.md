...
## ★ 최신 상태 (2026-05-26, v0.3) — 여기부터 읽으세요

로컬 React(Vite + TS + Tailwind) 이식은 완료됨. 첫 근무 후 **정적 JSON → Firebase 전환**을 결정. 상세 배경·데이터모델은 `CHANGE_v0.3_DB전환.md` 참조. (아래 본문은 v0.2 시점 기록 — 데이터 구조 등 참고용으로 유효)

### 지금부터 할 작업 (확정된 순서)

1. **LLM(Anthropic) 기능 잠시 비활성화** — 두 키 보안을 동시에 다루지 않기 위해
2. Firebase 프로젝트 생성 + Firestore 셋업 (사용자가 콘솔에서 직접)
3. `mammoth_grouped.json` → Firestore `menus` 컬렉션 마이그레이션 (1회성 스크립트)
4. 앱 데이터 로딩: 정적 JSON → Firestore 읽기로 교체
5. Firebase Auth(본인 로그인) + Security Rules로 권한 설정
6. `/admin` 관리자 페이지에서 CRUD 구현
7. 첫 근무 확보 레시피로 데이터 갱신 (`verified: true`)
8. `guidelines` 컬렉션 + 운영지침 뷰 추가
9. 마지막: 서버리스 함수로 LLM Q&A 재연결 (Anthropic 키 숨김)

### 확정된 결정

- **권한**: 읽기 공개 + 쓰기는 본인만 (`allow read: if true; allow write: if request.auth != null;`)
- **CRUD UI**: 별도 `/admin` 페이지, 로그인 필요
- **지침**: 메뉴 DB 전환 완료 후 추가
- **Firebase 키**: 프론트엔드 노출 무방 (식별자 성격, 보안은 Security Rules로)
- **Anthropic 키**: 서버리스 함수로 숨김 + Vercel 환경변수 저장 (절대 프론트 노출 금지)
- **신규 데이터 필드**: `verified`(실측 확인), `updated_at`(수정 시각)

### 사용자가 직접 해야 하는 일 (Claude Code가 대신 못 함)

- Firebase 콘솔에서 프로젝트 생성, Firestore/Auth 활성화
- Firebase 설정값을 `.env`에 입력
- Vercel 대시보드에서 환경변수(Anthropic 키) 설정
- 배포는 GitHub push 후 Vercel 자동 빌드
